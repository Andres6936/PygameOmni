    ...
    for y in range(0, int(yCord / 16)):
        for x in range(0, int(xCord / 16)):
            ...
            if x == 0 or y == 0 or y == (yCord / 16) - 1 or x == (xCord / 16) - 1:
                cave[y][x] = Tile(passable=False, digable=False, position=(x * 16, y * 16), screen=screen,
                                  tile_image=wall_image)
            # Make ground tile
            elif random.randint(0, 100) > FILLFACTOR:
                cave[y][x] = Tile(passable=True, digable=True, position=(x * 16, y * 16), screen=screen,
                                  tile_image=ground_image)
            # Make wall tile
            else:
                cave[y][x] = Tile(passable=False, digable=True, position=(x * 16, y * 16), screen=screen,
                                  tile_image=wall_image)
